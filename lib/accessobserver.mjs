...
import { isNil, isString, isSymbol, isRef, isObject } from "/evolux.util/lib/objutils.mjs";
...
export default class AccessObserver extends BaseObserver {
    ...
    addEventListener(eventname, listener, options) {
        ...
        if (!listener._x) listener._x = 'l_' + rnd(8);
        let listeners = this._eventListeners[eventname];
        if (!listeners) this._eventListeners[eventname] = listeners = [];
        if (listeners.indexOf(listener) === -1) listeners.push(listener);
        // console.log('### AccessObserver#addEventListener', eventname, this._x, this.soul ?? 'NONTHOREGON', listener._x, this._eventListeners[eventname].length);
    }
        ...
        (async (listeners, eventname, details, opt) => {
            ...
            listeners?.forEach(listener => {
                try {
                    ...
                    // console.log('### AccessObserver#emit', eventname, this._x, this.soul ?? 'NONTHOREGON', listener._x);
