...
export default class AccessObserver extends BaseObserver {
    ...
    __addDeepListenersOnProperties__(options = {}) {
        const properties = Reflect.ownKeys(this.target).filter((prop) => !isPrivateProperty(prop)); // all existing properties w/o private
        const receiver   = this.proxy$;
        if (!options.entities) {
            options.entities = new Set();
            options.entities.add(this);
        }
    ...
    removeDeepListener(eventname, listener, options = {}) {
        ...
        this.__removeDeepListenerOnProperties__(options);
    }

    __removeDeepListenerOnProperties__(options = {}) {
        const properties = Reflect.ownKeys(this.target).filter((prop) => !isPrivateProperty(prop)); // all existing properties w/o private
        const receiver   = this.proxy$;
        ...
        properties.forEach((prop) => {
            const value = receiver[prop];
            if (options.entities.has(value)) return;
            value?.__checkRemoveDeepListeners__?.(receiver, prop, options);
        });
    }

    __checkRemoveDeepListeners__(parent, prop, options = {}) {
