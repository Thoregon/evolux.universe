...
export default class BrowserLoader extends Reporter() {
    ...
    static serve({
        ...
        let loader = new this({root, index, embedded, cachelocation, cachecontrol, moduleroot, scriptroot, port});
        ...
        loader.setupExpress(port, host, greenlock);
     ...
     */
    setupExpress(port, host, greenlockcfg) {
        service.use(cors({
        ...
        if (greenlockcfg) {
            ...
            console.log("$$ BrowserLoader: greenlock serve");
            // universe.wwwservice = wrapper;
        } else {
            universe.wwwservice = service;
            // now start the service
            service.listen(port, host);
            console.log("$$ BrowserLoader: express serve");
        }
    }
    ...
    http(req, res, next) {
        ...
        if (this.cachecontrol /*&& !res.getHeader('Cache-Control')*/) res.setHeader('Cache-Control', this.cachecontrol);
        ...
        if (url.startsWith('/widgets'))                 return this.sendWidgets(req, res);
    ...
    sendWidgets(req, res) {
        let widgets = req.query.name;
        if (!res.getHeader('content-type')) res.setHeader('Content-Type', "application/javascript; charset=UTF-8");
        res.send(ModuleResolver.buildWidgetScript(widgets));
    }
