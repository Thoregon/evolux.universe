...
export default class BrowserLoader {
   ...
   static serve({
    ...
    isModule(url) {
        return url !== '/' && url.startsWith("/") && fs.existsSync(path.join(process.cwd(), this.moduleroot, url.substr(1)));
    }
    ...
    async sendModule(url, req, res) {
        try {
            ...
            if (!bundlefilepath) {
                let module = moduleresolver.resolveModule(modulename, this.moduleroot);
                // todo: build ES6 if 'node'
                bundlefilepath = module.mainpath;
            }
