...
import {myuniverse, timeout}    from "../thoregonhelper.mjs";
...
export default class BrowserLoader extends Reporter() {
    ...
    setupExpress(port, greenlockcfg) {
        ...
        if (greenlockcfg) {
            ...
            myuniverse().wwwservice = service;
            // myuniverse().wwwservice = wrapper;
        } else {
            myuniverse().wwwservice = service;
    ...
    // todo: refactor, build a better redirect structure to handle more requests with the same logic
    http(req, res, next) {
        let fpath;
        let url     = req.originalUrl;
        let referer = req.header("referer");
//        if (!res.getHeader('Referrer-Policy')) res.setHeader('Referrer-Policy', "same-origin, no-referrer-when-downgrade");
