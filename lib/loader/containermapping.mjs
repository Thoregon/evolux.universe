...
export default async () => {
    ...
    try {
        let cwd = process.cwd();
        let modules = await pfs.readdir(path.resolve('modules'));

        for await (const module of modules) {
            const containermodules = path.resolve('modules');
            const root = path.resolve('modules', module);
            let entries = await exploreModule(root);
            // module = module === 'www' ? module : `/${module}`;
            if (entries) Object.assign(c, { [module]: { fs: containermodules, entries } });
        }
/*
