...
import m from './containermapping.mjs'
...
export default class Bootloader {
    ...
    async resolve(specifier, context, nextResolve) {
        ...
        let resolved;
        if (loader) {
            resolved = await loader.resolve(specifier, context, nextResolve);
        } else {
            try {
                resolved = await nextResolve(specifier, context, nextResolve);
            } catch (e) {
                // redirect to 'node_modules' of the thoregon installation
                resolved = await nextResolve(specifier, {
                    parentURL: this.getDefaultParentURL(),
    ...
    getDefaultParentURL() {
        return "file:///" + process.cwd() + "/thoregon.mjs";
    }
    ...
    async load(url, context, nextLoad) {
        ...
        return (loader && loader.load)
            ? await loader.load(url, context, nextLoad)
...
class LoaderTemplate {
    ...
    async resolve(specifier, context, nextResolve) {
        return {
            url: '',            // The absolute URL to which this input resolves
            format: '',         // A hint to the load hook (it might be ignored) 'builtin' | 'commonjs' | 'json' | 'module' | 'wasm'
...
class TDevLoader {
    ...
    constructor(props) {
        this._ready = false;
        this.onReady = () => {
        };        // set this handler to handle ready file mapping
        this.explore();
    }
    ...
    async resolve(specifier, context, nextResolve) {
        ...
        if (!ref) return nextResolve(specifier, context, nextResolve);
        return {
            url: isFile(path.join(ref.fs, ref.path)) ? 'file://' + path.join(ref.fs, ref.path) : (isFile(`${path.join(ref.fs, ref.path)}/index.mjs`)) ? `file://${path.join(ref.fs, ref.path)}/index.mjs` : `file://${path.join(ref.fs, ref.path)}/index.js`,
            shortCircuit: true  // NodeJS 18.x
        };
    }
...
class ContainerLoader extends TDevLoader {
...
loaders.push({name: 'devcomponent', loader: new ContainerLoader()});
