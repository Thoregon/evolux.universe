...
class Universe extends DeferredProperties(Object) {
    ...
    constructor() {
        ...
        this.$universe          = this;
        this.random             = rnd;
        this.util               = { ...util, doAsync, timeout, halt };
    ...
    debuglog(id, msg, ...args) {
        if (!this.logDbgId(id)) return;
        if (this.DEBUGCONSOLE) this.logger.log(id, this.inow, ':', msg, ...args);
        const logentry = { id, dttm: this.inow, msg, args };
    ...
    get debugids() {
        if (!DBGIDS) DBGIDS = new Set(this.DEBUGIDS ?? []);
        return DBGIDS;
    }
