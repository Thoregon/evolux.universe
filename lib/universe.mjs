...
import { className }  from '/evolux.util/lib/utilfns.mjs';
 ...
 *
 */
export default class Universe extends DeferredProperties(Object) {

    constructor(dependencies) {
        super();
        this._inflated = false;

        const unload = dependencies.unload ?? { add: () => {}, uncaught: () => {}};
        const dataurl = dependencies.dataurl ?? {};
        ...
        this._stopListenDusk    = unload.add(async (code) => await this.freeze(code));
        ...
        if (globalThis.process) {
            this.nodeVersion        = parseInt(process.versions.node.split('.')[0]);
        }
        this.util               = { doAsync, timeout, halt, ...dataurl };
    ...
    buildPub(fpath) {
        const dir = this.$universe._pubrootdir();
        const ppath = this.path.join(dir, fpath);
        console.log(">> requested PUB:", ppath);
        return ppath;
    }

    _pubrootdir() {
        return this.PUB_DIR ?? this.env.pubroot ?? './pub';
    }
...
*/
    ...
    async letThereBeLight() {
        ...
        try {
            ...
            for await (const fn of fns) await fn(this);
            ...
            for await (const [name, item] of Object.entries(this)) {
    ...
    async freeze(code) {
        ...
        let proms = [];
        try {
            for await (const fn of fns) await fn(this, code);
    ...
    async module(path) {
        return await thoregon.import(path);
    }
