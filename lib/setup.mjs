...
const getPeerConfigFiles = (basedir) => {
    let find = /^peer\..*config\.mjs$/;
    if (isDirectory(basedir)) {
        let dircontent = fs.readdirSync(basedir);
        return dircontent.filter(item => item.match(find)).map(filename => path.join(basedir, filename));
    }
    return [];
};
...
let setup = ((configroot) => {
    ...
    let peercfgfiles = getPeerConfigFiles(_env.basedir);
