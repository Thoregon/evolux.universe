    ...
    for (let filename of filenames) {
        ...
        if (fs.existsSync(universeFile)) {
            ...
            if (filename === DEFAUTLT_CONFIG_FILENAME) {
                logger.log("info", "default config not available: '%s'", filename);
            } else {
                logger.log("warning", "Stage (%s) config not found: '%s'", stage, filename);
            }
        }
    }
    return resolved;
...
let Setup = (() => {
    ...
    if (!!argv.cfg) {
        if (!fs.existsSync(argv.cfg)) {
            logger.log("error", "unknown config: '%s'", argv.cfg);
            process.exit(1);
        }
    ...
    _env.universeFiles = getUniverseFiles(_env.stage, _env.basedir, _env.universe);

    for (let cfgfile of _env.universeFiles) {
        // check if config exists
        if (!fs.existsSync(cfgfile)) {
            logger.log("error", "unknown config: '%s'", cfgfile);
        } else {
            let stats = fs.statSync(cfgfile);
            if (!stats.isFile()) {
                logger.log("error", "config is not a file: '%s'", cfgfile);
                process.exit(1);
            }
        }
    }
...
/*
    ...
    function getUniverseFiles(basedir, filenames) {
        let resolved = [];
        for (filename in filenames) {
            ...
            let cfgfile;
            while (!cfgfile && dir.startsWith(basedir) && !fs.existsSync(universeFile)) {
                dir = path.join(dir, '../');
                universeFile = path.join(dir, filename);
                if (fs.existsSync(universeFile)) cfgfile = universeFile;
            }
            if (cfgfile) resolved.push(cfgfile);
        }
    }
 */
