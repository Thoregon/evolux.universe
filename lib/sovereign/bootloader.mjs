...
const JS_EXTENSIONS     = new Set(['.js', '.mjs', '.json', '.tcd']);    // todo: enable extensions with bootloader plugin
const thoregonloader    = new ThoregonLoader({ root: './', cachelocation: './.thoregon/jscache'});

// polyfill localStoreage
if (!global.localStorage)   Object.defineProperty(global, "localStorage", { value: new nodels.LocalStorage('./.thoregon/localStore'),  configurable: false, enumerable: true, writable: false });
...
export default class Bootloader {
    ...
    async resolve(specifier, parentModuleURL = baseURL, defaultResolve) {
        ...
        if (specifier === './@components') return this.resolveComponents(specifier,parentModuleURL);
        ...
        // if (this.isNodeModuleFile(specifier, basename)) return this.resolveNodeModuleFile(specifier, parentModuleURL, defaultResolve);
...
/*
    resolveNodeModuleFile(specifier, parentModuleURL, defaultResolve) {
