...
const componentdirs = ['sovereign', 'reliant', 'headless', 'headed', 'rich', 'lite', 'ui'];
const hasUI         = (mode) => /*universe.UI ||*/ mode.nature === 'ui' || mode.nature === 'reliant' || mode.density === 'headed';
...
export default class ModuleResolver {
    ...
    static findModuleMain(modulepath) {
        let namei = modulepath.lastIndexOf("/"),
            name  = (namei > -1) ? modulepath.substr(namei + 1) : modulepath;
        ...
        if (stat.isFile()) {
            ...
            if (fs.existsSync(bowerpath)) {
                ...
                // reference to a esnext module
                } else if (packagedefinition["esnext"]) {
                    moduleref.format = 'module';
                    moduleref.url = path.join(modulepath, packagedefinition["esnext"]);
                // reference to a ES6 module
                ...
                // reference to a esnext module
                } else if (packagedefinition["module"]) {
                    moduleref.format = 'module';
                    moduleref.url = path.join(modulepath, packagedefinition["module"]);
                    // reference to a ES6 module
    ...
    static async wrapES6(name, moduleref, cachelocation) {
        ...
        let i = wrapper.indexOf('§§modulesource§§');
        modulesource = wrapper.substring(0, i) + modulesource + wrapper.substring(i+16);
    ...
    static async buildComponentsScript(referer, location, cacheroot, mode) {
        ...
        let baseurl = path.join(refurl.pathname, '..');
        let root = location, url = '/';
        // if (baseurl === '/') baseurl = refurl .pathname;
    ...
    static async analyseComponents(url, root, mode) {
        ...
        let ri = root.indexOf('/components');
        if (ri > -1) rootlen = ri+1;
        ...
        if (isDirectory(url)) {
            ...
            if (hasUI(mode)) await this._componentsWithin(components, path.join(url, `/ui`), rootlen);   // there can be UI components available for sovereign headed and (all) reliant nodes
            if (mode.nature) {
            ...
            if (!componentdirs.includes(name) && isDirectory(path.join(url, name))) {
                ...
                try {
                    ...
                    if (!componentdescriptor.default) {
                        ...
                        let compid = componentdescriptor.default.id;
                        if (compid) {
    ...
    static buildWidgetScript(widgets) {
        if (!Array.isArray(widgets)) widgets = [widgets];
        let src, widgetclspath;

        // get base classes for widget
        widgetclspath = thoregonloader.resolveUrl('/thoregon.widget/lib/thoregonwidget.mjs');
        src = fs.readFileSync(widgetclspath).toString();

        // get specified widgets
        widgets.forEach(widget => {
            widgetclspath = path.join(process.cwd(), 'components/widgets', widget.toLowerCase()+'.mjs');
            src += '\n\n/* ' + widget + ' */' + fs.readFileSync(widgetclspath).toString();
        });
