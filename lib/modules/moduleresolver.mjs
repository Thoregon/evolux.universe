...
export default class ModuleResolver {
    ...
    static async analyseComponents(url, root, mode) {
        ...
        if (isDirectory(url)) {
            this._componentsWithin(components, url, rootlen);
            if (mode.nature) {
                this._componentsWithin(components, path.join(url, `/${mode.nature}`), rootlen);
                if (mode.density) {
                    this._componentsWithin(components, path.join(url, `/${mode.nature}/${mode.density}`), rootlen);
                }
            }
        }
        return components;
    ...
    static _componentsWithin(components, url, rootlen) {
